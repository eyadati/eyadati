import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

// Professional clinic blue for interactive elements - original HSL: (210, 0.75, 0.55)
// This is an approximation in RGB. You can fine-tune this if needed.
const Color _primarySeedColor = Color.fromARGB(255, 34, 139, 230);

ThemeData darkMode = ThemeData(
  brightness: Brightness.dark,
  colorScheme: ColorScheme.fromSeed(
    seedColor: _primarySeedColor,
    brightness: Brightness.dark,
    // Using darker grey for surface and background in dark mode
    surface: const Color(0xFF121212), // Dark grey
    onSurface: const Color(0xFFE0E0E0), // Light grey for text on surface
    error: const Color(0xFFCF6679), // Standard red for error
    onError: const Color(0xFF121212), // Dark grey text on error
  ),
  // App bar styling
  appBarTheme: AppBarTheme(
    backgroundColor: const Color(0xFF121212), // Dark grey
    foregroundColor: const Color(0xFFE0E0E0), // Light grey
    elevation: 0,
  ),
  // Scaffold background
  scaffoldBackgroundColor: const Color(0xFF0A0A0A), // Even darker grey
  // Card styling
  cardTheme: CardThemeData(
    color: const Color(0xFF1E1E1E), // Slightly lighter dark grey for cards
    elevation: 1,
    margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
  ),
  // Button styling
  elevatedButtonTheme: ElevatedButtonThemeData(
    style: ElevatedButton.styleFrom(
      // Background will come from colorScheme.primary
      // Foreground will come from colorScheme.onPrimary
      padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 16),
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
    ),
  ),
  // Input fields
  inputDecorationTheme: InputDecorationTheme(
    filled: true,
    fillColor: const Color(0xFF1E1E1E), // Dark grey for input fields
    border: OutlineInputBorder(
      borderRadius: BorderRadius.circular(12),
      borderSide: const BorderSide(
        color: Color(0xFF333333), // Darker grey border
      ),
    ),
    focusedBorder: OutlineInputBorder(
      borderRadius: BorderRadius.circular(12),
      borderSide: BorderSide(
        color: _primarySeedColor, // Use the primary seed color for focused
        width: 2,
      ),
    ),
  ),
  // Text styling
  textTheme: GoogleFonts.interTextTheme(
    ThemeData.dark().textTheme.copyWith(
      bodyLarge: const TextStyle(
        color: Color(0xFFAFAFAF), // Light grey for body text
        height: 1.5,
      ),
      bodyMedium: const TextStyle(
        color: Color(0xFFAFAFAF), // Light grey for body text
        height: 1.5,
      ),
      titleLarge: const TextStyle(
        color: Color(0xFFE0E0E0), // Lighter grey for titles
        fontWeight: FontWeight.w600,
      ),
      // Ensure other text styles also have good contrast
    ),
  ),
);
