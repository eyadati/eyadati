...
class FakeDocumentSnapshot implements DocumentSnapshot<Map<String, dynamic>> {
  ...
  bool operator ==(Object other) =>
      other is FakeDocumentSnapshot && other.id == id;
  @override
...
void main() {
  group('UserAppointmentsProvider', () {
    ...
    late mocks.MockCollectionReference<Map<String, dynamic>>
    mockUsersCollection;
    ...
    setUp(() {
      ...
      mockUsersCollection =
          mocks.MockCollectionReference<Map<String, dynamic>>();
      ...
      when(
        mockFirestore.collection('clinics'),
      ...
      when(
        mockUserDoc.collection('appointments'),
      ).thenReturn(mockAppointmentsCollection);

      // Mock the query chain
      when(
        mockAppointmentsCollection.where(
          'date',
      ...
      final mockClinicDocRef1 =
          mocks.MockDocumentReference<Map<String, dynamic>>();
      when(mockClinicDocRef1.get(any)).thenAnswer((_) async {
        return FakeDocumentSnapshot('clinic_id_1', true, {
          'name': 'Clinic One',
          'FCM': 'fcm_token_1',
          'duration': 60,
        });
      ...
      when(
        mockClinicsCollection.doc('clinic_id_1'),
      ).thenReturn(mockClinicDocRef1);

      final mockClinicDocRef2 =
          mocks.MockDocumentReference<Map<String, dynamic>>();
      when(mockClinicDocRef2.get(any)).thenAnswer((_) async {
        return FakeDocumentSnapshot('clinic_id_2', true, {
          'name': 'Clinic Two',
          'FCM': 'fcm_token_2',
          'duration': 90,
        });
      ...
      when(
        mockClinicsCollection.doc('clinic_id_2'),
      ).thenReturn(mockClinicDocRef2);
      ...
      when(mockClinicsCollection.doc(any)).thenAnswer((Invocation inv) {
        ...
        when(mockDocRef.get(any)).thenAnswer((_) async {
          if (id == 'clinic_id_99') {
            // Handle specific test case for clinic_id_99
            return FakeDocumentSnapshot('clinic_id_99', true, {
              'name': 'Found Clinic',
              'FCM': 'fcm_99_token',
            });
      ...
      when(
        mockUserFirestore.cancelAppointment(any, any, any),
      ).thenAnswer((_) async => {});

      // Mock NotificationService.sendDirectNotification
      when(
        mockNotificationService.sendDirectNotification(
    ...
    test('loadAppointments fetches and updates appointments', () async {
      ...
      final mockDocSnap1 =
          mocks.MockQueryDocumentSnapshot<Map<String, dynamic>>();
      when(mockDocSnap1.id).thenReturn('app_id_1');
      when(
        mockDocSnap1.data(),
      ).thenReturn({'clinicUid': 'clinic_id_1', 'date': now});
      ...
      expect(
        provider.hasMore,
        isFalse,
      ); // Changed to isFalse due to _pageSize logic
    ...
    test('refresh method clears and reloads appointments', () async {
      ...
      when(
        mockQuery.get(any),
      ).thenAnswer((_) async => initialSnapshot); // First call to get()
      ...
      when(
        mockQuery.get(any),
      ).thenAnswer((_) async => refreshedSnapshot); // Subsequent calls to get()
    ...
    test(
      'cancelAppointment removes appointment and sends notification',
      () async {
        final now = Timestamp.now();
    ...
    test('getClinicData returns null if clinic not found', () async {
      final mockDocForNonExistentClinic =
          mocks.MockQueryDocumentSnapshot<Map<String, dynamic>>();
      when(mockDocForNonExistentClinic.id).thenReturn('app_id_x');
      when(mockDocForNonExistentClinic.data()).thenReturn({
        'clinicUid': 'non_existent_clinic',
        'date': Timestamp.now(),
      });

      when(mockQuerySnapshot.docs).thenReturn([mockDocForNonExistentClinic]);
    ...
    test('getClinicData returns data if clinic found', () async {
      final mockAppDocSnap =
          mocks.MockQueryDocumentSnapshot<Map<String, dynamic>>();
      when(mockAppDocSnap.id).thenReturn('app_id_y');
      when(
        mockAppDocSnap.data(),
      ).thenReturn({'clinicUid': 'clinic_id_99', 'date': Timestamp.now()});
    ...
    test('pagination loads more appointments', () async {
      ...
      final List<mocks.MockQueryDocumentSnapshot<Map<String, dynamic>>>
      firstBatchDocs = [];
      for (int i = 0; i < 20; i++) {
        final mockDocSnap =
            mocks.MockQueryDocumentSnapshot<Map<String, dynamic>>();
      ...
      final secondBatchQuerySnapshot =
          mocks.MockQuerySnapshot<Map<String, dynamic>>();
      when(
        secondBatchQuerySnapshot.docs,
      ).thenReturn([secondBatchDocSnap21]); // Only one doc for the second batch

      final mockQueryAfterStart = mocks.MockQuery<Map<String, dynamic>>();
      when(
        mockQueryAfterStart.get(any),
      ).thenAnswer((_) async => secondBatchQuerySnapshot);
      ...
      expect(
        provider.hasMore,
        isTrue,
      ); // Should be true now as 20 docs were returned
      ...
      expect(
        provider.hasMore,
        isFalse,
      ); // Should be false as only 1 doc was returned in second batch
    });
